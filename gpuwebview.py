...
__email__ = f'Skyler.He@richmond.edu'
...
async def websocket_endpoint(websocket: WebSocket):
    """
    WebSocket endpoint to stream GPU stats to clients.
    """
    ...
    try:
        while True:
            gpu_data = gather_data(myargs)  # Get updated GPU data
            for client in connected_clients:
                await client.send_json(gpu_data)
            await asyncio.sleep(myargs.time)  # Wait before sending the next update
